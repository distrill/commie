def decodeNumbers(filename: str):
    with open(filename, 'r') as file:
        ...
        return sum(len(list(filter(lambda x: x, [True if len(stringNumber) in validNumbers else False for stringNumber in numberLine]))) for numberLine in numberLines)
